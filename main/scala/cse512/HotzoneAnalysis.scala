...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    // val countDf = spark.sql("select joinResult.rectangle as rectangle, COUNT(*) as count from joinResult GROUP_BY(joinResult.rectangle)")
    // countDf.createOrReplaceTempView("countResult")
    val countrectDf = joinDf.orderBy("rectangle").groupBy("rectangle").count()

    return countrectDf // YOU NEED TO CHANGE THIS PART
  }
